    ...
    stages{
        stage('Pre-Build'){
		    ...
		    mergeid=merge_mesg.split(':')[0];
                    echo "${mergeid}"
                    sh "curl -s -H \"Authorization: token fd565fbb695a99edef251944f0e55c96a4e61e27\"  -X POST -d '{\"body\": \"https://jira-url\"}'  \"https://api.github.com/repos/sarulatha333/first/issues/8/comments\""
